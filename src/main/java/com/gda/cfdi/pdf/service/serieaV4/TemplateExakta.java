...
public class TemplateExakta  extends PdfPageEventHelper{
	...
	public TemplateExakta(PdfInfoDto infoPDF, Comprobante comprobante, boolean retencion, boolean bDirFiscal, Environment env) throws Exception{
		try {
			if(retencion){
...
//			CRegimenFiscal cregimenfiscal = emisor.getRegimenFiscal();
			...
			imagenLogo = Image.getInstance(env.getProperty("path.file.ordenes.logo.exakta"));
...
//            tabDetalleFact.addCell(pcLineasII);
            ...
            File f = new File(env.getProperty("path.file.ordenes.qr.exakta"));
	      	GenerarQRCode qrCode = new GenerarQRCode();
	      	qrCode.generateQR(f, imgCrearQr, 600, 600);
          
      		imagenQr = Image.getInstance(env.getProperty("path.file.ordenes.qr.exakta"));
