package com.gda.cfdi.pdf.service.exakta;
...
@Service
public class PdfExaktaService {

	private static final Logger log = LoggerFactory.getLogger(PdfExaktaService.class);
	...
	public String generarPdfExaktaV40(InformacionPdfBean cfdiBean) throws DocumentException, IOException, Exception {
		...
		if(!cfdiBean.getXml().isEmpty() && cfdiBean.getXml().trim().length()>5) {
			...
			if(comprobante.getTipoDeComprobante() == CTipoDeComprobante.I || 
				...
				ruta = iTemplatePdfServiceV4.CrearPdfMarcaExaktaEmpresa(comprobante, infoPDF, env);				
