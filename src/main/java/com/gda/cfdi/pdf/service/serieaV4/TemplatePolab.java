...
public class TemplatePolab  extends PdfPageEventHelper{
	...
	public TemplatePolab(PdfInfoDto infoPDF, Comprobante comprobante, boolean retencion, boolean bDirFiscal, Environment env) throws Exception{
		try {
			if(retencion){
...
//            tabDetalleFact.addCell(pcLineasII);
            ...
            File f = new File(env.getProperty("path.file.ordenes.qr.polab"));
	      	GenerarQRCode qrCode = new GenerarQRCode();
	      	qrCode.generateQR(f, imgCrearQr, 600, 600);
          
      		imagenQr = Image.getInstance(env.getProperty("path.file.ordenes.qr.polab"));
