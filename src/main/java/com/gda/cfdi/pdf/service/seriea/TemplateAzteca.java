...
public class TemplateAzteca extends PdfPageEventHelper{
	...
	public TemplateAzteca(PdfInfoDto infoPDF, Comprobante comprobante, boolean retencion, boolean bDirFiscal, Environment env) throws Exception{
		try {
			if(retencion){
...
//            PdfPCell pcCFDI = new PdfPCell(new Paragraph("G03 Gastos en General",fuenteImport));
            ...
            Chunk datoDomicilioReceptor = null;
            if(infoPDF.getAdendaDireccion()!=null){
           	 datoDomicilioReceptor = new Chunk(infoPDF.getAdendaDireccion(),fuenteContenidoImporTab);            	
           }else{
           	datoDomicilioReceptor = new Chunk("",fuenteContenidoImporTab);
           }
...
//            tabDetalleFact.addCell(pcNumOrdenPaciente);
          ...
          //uuidRelacionado
            ...
            PdfPCell pcCertEmisor = new PdfPCell(new Paragraph("Certificado del emisor: "+comprobante.getNoCertificado()+
            		"\t RFC del Proveedor de Certificaci�n: "+timbreFiscalDigital.getRfcProvCertif(),fuenteTimbradoImpor));
