...
public class TemplateSwissComplemento  extends PdfPageEventHelper{
...
//	float[] medidaCeldas = {0.5f};
	private static final Logger log = LoggerFactory.getLogger(TemplateSwissComplemento.class);
	public TemplateSwissComplemento(Comprobante comprobante, ComplementoDatosDto complementoDatos, String serie, Environment env) throws Exception{
		try {
...
//				uuidRelacionado = comprobante.getUuidRelacionado();
			...
			Font fuenteTimbradoImpor = new Font(Font.FontFamily.HELVETICA,4,Font.BOLD,BaseColor.BLACK);
			 ...
			 if(serie.equals("ASL")){
				 imagenLogo = Image.getInstance(env.getProperty("path.file.ordenes.logo.liacsa"));
			 }else{				 
				 imagenLogo = Image.getInstance(env.getProperty("path.file.ordenes.logo.swisslab"));
			 }
            ...
            if(serie.equals("ASL")){
            	direccionSucursal = new PdfPCell(new Paragraph("C. GENERAL RETANA 905, SAN FELIPE C.P. 31203, CHIHUAHUA, CHIHUAHUA",fuenteDirSucur));
            }else{
            	direccionSucursal = new PdfPCell(new Paragraph("C. NICOLAS BRAVO 725, MARIA LUISA, 64040, MONTERREY, NUEVO LEÓN",fuenteDirSucur));
            }
