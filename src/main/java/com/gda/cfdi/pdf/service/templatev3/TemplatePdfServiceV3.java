...
@Service
public class TemplatePdfServiceV3 implements ITemplatePdfServiceV3Impl {
	
	private static final Logger log = LoggerFactory.getLogger(TemplatePdfServiceV3.class);
...
//	        }
	...
	@Override
	public String CrearPdfMarcaFamilyLabsNorte(Comprobante comprobante, PdfInfoDto infoPDF, Environment env) throws DocumentException, IOException, Exception  {
		...
		FileOutputStream ficheroPdf = null;
		try {
			/*rutaproduccion*/ ruta = env.getProperty("path.files.pdf.familylabsnorte")+"FNFA_"+infoPDF.getKfactura()+".pdf";
			/*rutapruebas*///ruta = "C:/Users/Desarrollo_GDA/documentos Timbrado/pdf/Jenner/pdf/JEFA_"+kfactura+".pdf";
			ficheroPdf = new FileOutputStream(ruta);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		...
		try {
			writerJenner = PdfWriter.getInstance(reporteAzteca, ficheroPdf);
			try {
				writerJenner.setPageEvent(new TemplateFamilyLabsNorte(comprobante,infoPDF, env));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
