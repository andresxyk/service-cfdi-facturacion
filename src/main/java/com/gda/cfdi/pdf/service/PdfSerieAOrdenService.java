...
public class PdfSerieAOrdenService {
	...
	@Autowired
	private com.gda.cfdi.pdf.service.serieaordenV4.CfdiPdfV4Service cfdiPdfServiceV4;
			...
			if(facturaDto!=null) {
				if(!xml.isEmpty() && xml.trim().length()>5) {
					...
					case 15:// Liacsa
						...
						ruta = cfdiPdfService.CrearPdfMarcaSwiss(infoPdf, kfactura, inicioNom+folioFactura, isRetencion, comprobante, bDirFiscal);
						break;
					case 19:// FamilyLabsNorte
						inicioNom = "FacturacionElectronica_AFN";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 6);
						ruta = cfdiPdfService.CrearPdfMarcaFamilyLabsNorte(infoPdf, kfactura, inicioNom+folioFactura, isRetencion, comprobante, bDirFiscal);
						break;
					default:
						break;
					}
...
//		      file.delete();
	...
	public String generarPdfOrdenV40(Integer kfactura,boolean isDescuento, String notaDescuento, boolean isRetencion) throws Exception {
		try {
			TFacturaDto facturaDto = consultaService.getTFacturaById(kfactura);
			String ruta = "";
			String xml = facturaDto.getXmlTimbrado();
			...
			if(facturaDto!=null) {
				if(!xml.isEmpty() && xml.trim().length()>5) {
					...
					case 5:// Swisslab
						...
						inicioNom = "FacturacionElectronica_AS";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 7);
						ruta = cfdiPdfServiceV4.CrearPdfMarcaSwiss(infoPdf, kfactura, inicioNom+folioFactura, isRetencion, comprobante, bDirFiscal);
						break;
					case 1:// Olab
						inicioNom = "FacturacionElectronica_A";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 8);					
						ruta = cfdiPdfServiceV4.CrearPdfMarcaOlabCfdi(infoPdf, kfactura, inicioNom+folioFactura, isDescuento, notaDescuento, comprobante, isRetencion,bDirFiscal);
						break;
					case 4:// Azteca
						inicioNom = "FacturacionElectronica_AZ";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 7);
						ruta = cfdiPdfServiceV4.CrearPdfMarcaAzteca(infoPdf, kfactura, inicioNom+folioFactura, isRetencion, comprobante, bDirFiscal);
						break;
					case 7:// Jenner
						inicioNom = "FacturacionElectronica_AJP";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 6);
						ruta = cfdiPdfServiceV4.CrearPdfMarcaJenner(infoPdf, kfactura, inicioNom+folioFactura, true, isRetencion, comprobante,bDirFiscal);
						break;
					case 8:// Jenner
						inicioNom = "FacturacionElectronica_AJL";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 6);
						ruta = cfdiPdfServiceV4.CrearPdfMarcaJenner(infoPdf, kfactura, inicioNom+folioFactura, false, isRetencion, comprobante,bDirFiscal);
						break;
					case 15:// Liacsa
						inicioNom = "FacturacionElectronica_ASL";
						folioFactura = utilsService.formatoFolio(Integer.parseInt(comprobante.getFolio()), 6);
