...
public class TemplateSwiss  extends PdfPageEventHelper{
	...
	public TemplateSwiss(PdfInfoDto infoPDF, Comprobante comprobante, boolean retencion, boolean bDirFiscal, Environment env) throws Exception{
		try {
			if(retencion){
...
//			CRegimenFiscal cregimenfiscal = emisor.getRegimenFiscal();
            ...
            File f = new File(env.getProperty("path.file.ordenes.qr.swisslab"));
	      	GenerarQRCode qrCode = new GenerarQRCode();
	      	qrCode.generateQR(f, imgCrearQr, 600, 600);
          
      		imagenQr = Image.getInstance(env.getProperty("path.file.ordenes.qr.swisslab"));
