...
public class TemplateJenner extends PdfPageEventHelper{
	...
	public TemplateJenner(PdfInfoDto infoPdf, Comprobante comprobant33, boolean retencion, 
			boolean bDirFiscal, Environment env) throws Exception{
		try {
			if(retencion){
			...
			String strDomicFiscalRecep = infoPdf.getAdendaDireccion();
...
//			String strNomOrdPac = comprobante.getConsecutivo()+" "+comprobante.getNombrepaciente();
           ...
           Chunk datoDomicilioReceptor = new Chunk(strDomicFiscalRecep,fuenteContenidoImporTab);
          ...
          Chunk DomicilioEmisor = new Chunk("Domicilio fiscal: ",fuenteContenidoTab);
           Chunk datoDomicilioEmisor = new Chunk(strDomicFiscalEmis,fuenteContenidoImporTab);
...
//            	System.out.println("UUIDRelacionadoOlab:::::     "+comprobante.getUuidRelacionado());
	            //uuidRelacionado
